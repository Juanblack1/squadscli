...
async function runTemplateProvider(name: ProviderName, template: string, request: RunRequest): Promise<ProviderResult> {
  ...
  if (name === "codex") {
    ...
    return await new Promise<ProviderResult>((resolve, reject) => {
      ...
      child.on("close", (code) => {
        ...
        if (code !== 0) {
          reject(new Error(stderr || `${name} falhou com codigo ${code}.`));
          return;
        }
        ...
        if (!text) {
          reject(new Error(`${name} nao retornou saida util.`));
          return;
        }

        resolve({
          text,
          raw: { stdout, stderr, code, promptFile, provider: name, mode: "stdin" },
        });
      });
    });
  }
  ...
  return await new Promise<ProviderResult>((resolve, reject) => {
    ...
    child.on("close", (code) => {
      ...
      resolve({
        text,
        raw: { stdout, stderr, code, promptFile, template: rendered, provider: name },
      });
    });
  });
}
  ...
  async invoke(_prompt: PromptBundle, request: RunRequest): Promise<ProviderResult> {
    ...
    try {
      ...
      for (const fallbackName of buildFallbackOrder(this.name)) {
        ...
        if (!fallbackTemplate || !fallbackBinary?.available) {
          continue;
        }
